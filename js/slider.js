...
$(window).on('load', function(){
  var $imgWidth = $('#slider_images19 img').first().outerWidth();
  var $imgCount = $('#slider_images19 img').length;
  $('#slider_images19').width($imgWidth*($imgCount+2));
  $('#slider_images19 img').first().addClass('endless_slider_first19');
  $('#slider_images19 img').last().addClass('endless_slider_last19');
  $('.endless_slider_first19').clone().appendTo('#slider_images19');
  $('.endless_slider_last19').clone().prependTo('#slider_images19');
  $('#slider_images19').css({'left':-1*$imgWidth+'px'});
   
  $('#right19').click(function(){
    $('#slider_images19').stop(true,true);
    var $newLeft = $('#slider_images19').position().left-(1*$imgWidth);
      var currentPos = Math.abs($newLeft)/$imgWidth;
      var currentCount = Math.round(currentPos);
      if (currentCount > $imgCount) {currentCount = currentCount - $imgCount}
      $('#count19').text(currentCount+'/'+$imgCount);
    $('#slider_images19').animate({'left':$newLeft+'px'},function(){
      var currentimg = Math.floor(Math.abs($newLeft));
      var lastimg = Math.floor(($imgCount+1)*$imgWidth);
       if (currentimg == lastimg) 
          {
  ...
  $('#left19').click(function(){
    ...
    $('#slider_images19').animate({'left':$newLeft+'px'},function(){
      var currentimg = Math.floor(Math.abs($newLeft));
       if (currentimg == (0)) 
          {
          $('#slider_images19').css({'left':-($imgCount)*$imgWidth+'px'});
          }
  });
   return false;
   });
